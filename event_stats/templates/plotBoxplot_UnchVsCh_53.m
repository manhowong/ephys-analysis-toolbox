...
% process the data.
    ...
    % for gramm().
g(1,1)=gramm('x',cellstr(inputFreq.challenge),'y',inputFreq.data,...
    'color',cellstr(inputFreq.challenge)); % cellstr to convert input data
                                            % to cells of string
...
g(1,1).set_color_options('map',[0 0.5 0;0 0 0]);

% Change x-axis data order
g(1,1).set_order_options("x",["Unchallenged","Challenged"]);
...
g(1,1).set_title('mEPSC Frequency - Adult, unmarked');

%% Define panels in the 2nd row of the figure

g(2,1)=gramm('x',inputFreq.data,'color',cellstr(inputFreq.challenge));
...
g(2,1).set_color_options('map',[0 0.5 0;0 0 0]);
...
g(3,1)=gramm('x',cellstr(inputAmpl.challenge),'y',inputAmpl.data,'color',cellstr(inputAmpl.challenge));
...
g(3,1).set_color_options('map',[0 0.5 0;0 0 0]);
g(3,1).set_order_options("x",["Unchallenged","Challenged"]);
g(3,1).geom_jitter();
%g(3,1).stat_summary('type','sem','geom',{'bar' 'black_errorbar'},'width',1);
g(3,1).stat_boxplot('width',1);
g(3,1).set_title('mEPSC amplitude - Adult, unmarked');

%% Define panels in the last row of the figure

g(4,1)=gramm('x',inputAmpl.data,'color',cellstr(inputAmpl.challenge));
...
g(4,1).set_color_options('map',[0 0.5 0;0 0 0]);
g(4,1).stat_bin('normalization','cdf','geom','stairs');

%% Plot all the panels on the same figure

figure('Position',[1000 100 640 800]); % create a figure and specify the position/size
...
h = findobj('Marker','o','MarkerEdgeColor','none','MarkerFaceColor',[0 0.5 0]);
set(h,'MarkerEdgeColor',[0 0.5 0],'MarkerFaceColor','none');