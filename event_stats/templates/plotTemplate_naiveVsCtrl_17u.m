...
% Sort data into two groups according to groupes and create two columns in
% the first row, each will contain a panel for one group.
g(1,1).facet_grid([],cellstr(inputFreq.group));
...
g(1,1).set_color_options('map',[1 0 0;1 0.6 0.6]);
...
g(2,1).facet_grid([],cellstr(inputFreq.group),"column_labels",0);
...
g(2,1).set_color_options('map',[1 0 0;1 0.6 0.6]);
...
g(3,1).facet_grid([],cellstr(inputAmpl.group));
...
g(3,1).set_color_options('map',[1 0 0;1 0.6 0.6]);
...
g(4,1).facet_grid([],cellstr(inputAmpl.group),"column_labels",0);
...
g(4,1).set_color_options('map',[1 0 0;1 0.6 0.6]);
